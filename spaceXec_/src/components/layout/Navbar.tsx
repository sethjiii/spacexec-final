  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-11/12 max-w-md max-h-[80vh] overflow-hidden border border-gray-200">
        <div className="flex justify-between items-center p-6 border-b border-gray-100" style={{ borderBottomColor: '#F2F1ED' }}>
          <h2 className="text-xl font-bold flex items-center" style={{ color: '#161616' }}>
        ...
        <div className="overflow-y-auto max-h-[60vh] p-6">
          ...
          {unreadNotifications.length === 0 && readNotifications.length === 0 && (
            <div className="text-center py-12">
              <FaBell className="w-12 h-12 mx-auto mb-4 opacity-40" style={{ color: '#B38F6F' }} />
...
const Navbar: React.FC = () => {
  ...
  const renderMobileMenu = () => (
    ...
    >
      <div className="h-full flex flex-col">
        <div className="p-6 flex justify-between items-center border-b" style={{ borderBottomColor: '#B38F6F' }}>
  ...
  const renderProfileMenu = () => (
    ...
    >
      <div className="p-6 border-b flex items-center" style={{ borderBottomColor: '#F2F1ED', backgroundColor: '#F2F1ED' }}>
        <img
          src={profilePic}
          alt="Profile"
        ...
        />
        <div>
          <p className="font-bold text-lg" style={{ color: '#161616' }}>{username}</p>
          <p className="text-sm" style={{ color: '#B38F6F' }}>View Profile</p>
        </div>
      </div>
      <div className="py-2">
        {[
          { to: `/dashboard/${userId}`, icon: FaUser, label: "Dashboard" },
          ...(userRole !== 'user' ? [{ to: `/channelpartner/${userId}`, icon: FaUser, label: "Channel Partner Dashboard" }] : []),
          ...(userRole === 'vendor' ? [{ to: `/vendordashboard`, icon: FaUser, label: "Vendor Dashboard" }] : []),
  ...
  const renderMoreNav = () => (
    <div
      className={`absolute z-50 right-0 top-full w-64 bg-white shadow-2xl rounded-2xl mt-3 overflow-hidden border transition-all duration-200 ${
        isMoreMenuOpen ? "opacity-100 visible transform translate-y-0" : "opacity-0 invisible transform -translate-y-2"
      }`}
      style={{ borderColor: '#F2F1ED' }}
    >
      <div className="py-2">
  ...
  return (
    <nav className="bg-white shadow-lg relative border-b-2" style={{ borderBottomColor: '#F2F1ED' }}>
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="hidden lg:flex items-center space-x-8">
            ...
            <div className="relative">
              ...
              >
                More
                <FaChevronDown className={`ml-2 w-3 h-3 transition-transform duration-200 ${isMoreMenuOpen ? 'rotate-180' : ''}`} />
                <span 
                  className="absolute bottom-0 left-0 w-0 h-0.5 transition-all duration-200 group-hover:w-full"
                  style={{ backgroundColor: '#710014' }}
                ></span>
              ...
              <div className="flex items-center space-x-4">
                <div className="relative">
                  ...
                  >
                    <FaBell 
                      className="w-5 h-5"
                      style={{ color: newNotificationCount > 0 ? '#710014' : '#B38F6F' }}
                    />
                    {newNotificationCount > 0 && (
                      <span
